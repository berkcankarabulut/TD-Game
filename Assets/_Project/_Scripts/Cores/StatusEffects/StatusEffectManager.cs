...
{
    public sealed class StatusEffectManager : MonoBehaviour
    {
        [SerializeField] private Unit _unit;

        private readonly Dictionary<SerializableGuid, StatusEffectInstance> _activeEffects =
            new Dictionary<SerializableGuid, StatusEffectInstance>();

        private readonly Dictionary<SerializableGuid, List<StatusEffectInstance>> _inactiveEffects =
            new Dictionary<SerializableGuid, List<StatusEffectInstance>>();
        ...
        {
            if (_unit.UnitHealth.AmIDead) return;
        ...
        {
            if (!_inactiveEffects.TryGetValue(effectId, out var inactiveInstances) ||
                inactiveInstances.Count == 0) return;
        ...
        {
            if (_unit == null) _unit = GetComponentInChildren<Unit>();
        }
#endif
    }
}